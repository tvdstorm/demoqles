...
str getSortName(appl(prod(label(_, lex(str n)), _, _), _)) = n;
...
list[Edit] iterDiff(Symbol s, list[Symbol] seps, Tree t1, Tree t2) {
  ...
  bool ptLCSEquals(Tree t1, Tree t2) {
    ...
    //    return true;
    //  }
    //  if (equalModuloKey(t1, t2)) {
    //    // to detect simple one-level renames
    //    return true;
    //  }
    //}
